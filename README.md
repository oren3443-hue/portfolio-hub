...
הכותרות בעברית מוגדרות באובייקט `OVERRIDES` בתוך `index.html`. כשנוסף ריפו חדש, הוא יופיע עם שם הריפו באנגלית עד שמוסיפים לו override.

**הדרך הקלה לערוך:** לחיצה על כפתור **"✏️ עריכת כותרות ב-GitHub"** ב-footer של האתר → פותח את הקובץ בעורך של GitHub. שמירה דורשת הרשאת כתיבה (יש רק לי).
